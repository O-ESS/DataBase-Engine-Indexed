...
//	}
	 ...
	 */
	public boolean deleteKeyAndRef(T key, Ref r, BPTreeInnerNode<T> parent, int ptr) {
		boolean done = false;
		for (int i = 0; !done && i < numberOfKeys; ++i) {
	...
	@Override
	public boolean updateKeyAndRef(T key, Ref oldR, Ref newR, BPTreeInnerNode<T> parent, int ptr) {
		boolean done = false;
		for (int i = 0; !done && i < numberOfKeys; ++i) {
			if (keys[i].compareTo(key) > 0)
				done = children[i].updateKeyAndRef(key, oldR, newR, this, i);
		}
		if (!done)
			done = children[numberOfKeys].updateKeyAndRef(key, oldR, newR, this, numberOfKeys);
		return done;
	}

	@Override
	public void insertRef(T key, Ref recordReference) {
		// TODO Auto-generated method stub
		children[findIndex(key)].insertRef(key, recordReference);
	}

	@Override
	public void deleteRef(T key, Ref recordReference) {
		// TODO Auto-generated method stub
		children[findIndex(key)].deleteRef(key, recordReference);
	}
