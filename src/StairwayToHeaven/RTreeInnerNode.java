...
public class RTreeInnerNode<T extends Comparable<T>> extends RTreeNode<T> implements Serializable {
	...
	public PushUpRT<T> insert(T key, Ref recordReference, RTreeInnerNode<T> parent, int ptr, TreeSet<point> inP) {
		...
		PushUpRT<T> pushUp = children[index].insert(key, recordReference, this, index, inP);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public RTreeInnerNode<T> split(PushUpRT<T> pushup) {
	...
	public Ref lowerBound(T key) {
		...
		return children[numberOfKeys].lowerBound(key);
	}
	 ...
	 *                  index
	 */
	public void insertLeftAt(int index, Comparable<T> key, RTreeNode<T> leftChild) {
		insertAt(index, key);
	 ...
	 *                   that index
	 */
	public void insertRightAt(int index, Comparable<T> key, RTreeNode<T> rightChild) {
		insertAt(index, key);
	 ...
	 * update Ref
	 */
//	public void updateRef(T key,Ref oldRef,Ref newRef, BPTreeInnerNode<T> parent, int ptr) {
//		boolean done = false;
//		for (int i = 0; i < numberOfKeys; ++i)
...
//	}
	...
	public boolean delete(T key, RTreeInnerNode<T> parent, int ptr, TreeSet<point> inP) {
		...
		for (int i = 0; !done && i < numberOfKeys; ++i)
			if (keys[i].compareTo(key) > 0)
				done = children[i].delete(key, this, i, inP);

		if (!done)
			done = children[numberOfKeys].delete(key, this, numberOfKeys, inP);
	...
	public Vector lowerThan(T key) {
		...
		for (int i = 0; i < numberOfKeys; ++i) {
			v.addAll(children[i].lowerThan(key));
		}
		v.addAll(children[numberOfKeys].lowerThan(key));
		return v;
	}
	...
	public Vector lowerThanOrEqual(T key) {
		Vector<Ref> v = new Vector<>();
		for (int i = 0; i < numberOfKeys; ++i) {
			v.addAll(children[i].lowerThanOrEqual(key));
		}
		v.addAll(children[numberOfKeys].lowerThanOrEqual(key));
		return v;
	}
	...
	public Vector biggerThan(T key) {
		Vector<Ref> v = new Vector<>();
		for (int i = 0; i < numberOfKeys; ++i) {
			v.addAll(children[i].biggerThan(key));
		}
		v.addAll(children[numberOfKeys].biggerThan(key));
		return v;
	}
	...
	public Vector biggerThanOrEqual(T key) {
		Vector<Ref> v = new Vector<>();
		for (int i = 0; i < numberOfKeys; ++i) {
			v.addAll(children[i].biggerThanOrEqual(key));

		}
		v.addAll(children[numberOfKeys].biggerThanOrEqual(key));
		return v;
	}

	/**
	 * delete key and ref and return true or false if it is deleted or not
	 */
	public boolean deleteKeyAndRef(T key, Ref r, RTreeInnerNode<T> parent, int ptr) {
		boolean done = false;
		for (int i = 0; !done && i < numberOfKeys; ++i) {
			// System.out.println(this+" "+done+" "+keys[i].compareTo(key)+"
			// "+numberOfKeys+" "+children[i]);
			if (keys[i].compareTo(key) >= 0)
				done = children[i].deleteKeyAndRef(key, r, this, i);

		}
		if (!done)
			done = children[numberOfKeys].deleteKeyAndRef(key, r, this, numberOfKeys);
		if (numberOfKeys < this.minKeys()) {
			if (isRoot()) {
				this.getFirstChild().setRoot(true);
				this.setRoot(false);
	 ...
	 */
	public void merge(Comparable<T> parentKey, RTreeInnerNode<T> foreignNode) {
	...
	public Vector getBucket(T key) {
		return children[findIndex(key)].getBucket(key);
	}
	...
	public boolean updateKeyAndRef(T key, Ref oldR, Ref newR, RTreeInnerNode<T> parent, int ptr, TreeSet<point> inP) {
		...
		for (int i = 0; !done && i < numberOfKeys; ++i) {
			if (keys[i].compareTo(key) > 0)
				done = children[i].updateKeyAndRef(key, oldR, newR, this, i, inP);
		}
		if (!done)
			done = children[numberOfKeys].updateKeyAndRef(key, oldR, newR, this, numberOfKeys, inP);
		return done;
	}
	...
	public void insertRef(T key, Ref recordReference, TreeSet<point> inP) {
		// TODO Auto-generated method stub
		children[findIndex(key)].insertRef(key, recordReference, inP);
	}

	@Override
	public void deleteRef(T key, Ref recordReference, TreeSet<point> inP) {
		// TODO Auto-generated method stub
		children[findIndex(key)].deleteRef(key, recordReference, inP);
	}

	@Override
	public boolean empty(T key) {
		// TODO Auto-generated method stub
		return children[findIndex(key)].empty(key);
	}
